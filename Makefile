...
# -------- 既存 DB 統合（Baseline + スキーマダンプ） --------

baseline-init: ## 既存 DB を Baseline で初期化（V1.0 まで手動適用済みと記録）
	@echo "⚠️  既存スキーマを Baseline として登録します"
	@echo "   このコマンドは、既に運用中の DB に Flyway を統合する場合に使用します"
	@read -p "Baseline Version を入力してください (デフォルト: 1.0): " version; \
	VERSION=$${version:-1.0}; \
	docker compose -f $(COMPOSE_FILE) run --rm flyway baseline \
	  -baselineVersion=$$VERSION \
	  -baselineDescription="Initial schema (manual setup)" && \
	echo "✅ Baseline initialized at version $$VERSION"

dump-schema: ## 既存テーブルをダンプして SQL に出力
	...
	echo "✅ Schema dumped to migrations/sql/V0__baseline.sql"
	@echo ""
	@echo "次のステップ:"
	@echo "1. migrations/sql/V0__baseline.sql を確認"
	@echo "2. make baseline-init で Baseline を初期化"
	@echo "3. make up で検証"

repair: ## Flyway checksum エラーを修復 (既存 SQL を変更してしまった場合)
	@echo "⚠️  WARNING: flyway repair は最後の手段です"
	@echo "   このコマンドは flyway_schema_history のチェックサムを再計算します"
	@echo "   通常は新規 migration ファイルで対応してください"
